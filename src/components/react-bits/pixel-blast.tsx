...
function disposeState(container: HTMLDivElement, state: ThreeState | null) {
  if (!state) return;
...
export function PixelBlast({
  ...
  useEffect(() => {
    ...
    else renderer.setClearColor(0x050711, 1);
    ...
    const onPointerMove = (event: PointerEvent) => {
      const touch = threeRef.current?.touch;
      if (!touch) return;
    ...
    const animate = (timestamp?: number) => {
      if (!autoPauseOffscreen || visible) {
        ...
        uniforms.uTime.value = timeOffset + timer.getElapsed() * speed;
        const liquidTime = liquidEffect?.uniforms.get('uTime') as THREE.Uniform<number> | undefined;
