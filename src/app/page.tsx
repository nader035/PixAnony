    ...
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:pt-6">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          {profile ? (
            <>
              ...
              <Link href="/paint" className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-contrast">Create art</Link>
            ...
            <>
              ...
              <Link href="/login?mode=signup" className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-contrast">Start creating</Link>
            </>
          )}
        </div>
        <button
      ...
      {open && (
        ...
        >
          ...
          >
            {profile ? 'Open feed' : 'Start creating'}
          </Link>
        </motion.div>
      )}
    </header>
      ...
      <div className="grid min-h-[540px] overflow-hidden rounded-[32px] bg-card p-4 shadow-[0_32px_100px_rgba(16,43,94,0.2)] md:grid-cols-[176px_minmax(0,1fr)_190px] md:p-5">
        <aside className="hidden flex-col border-r border-border/60 pr-4 md:flex">
          ...
          <div className="mt-3 space-y-1">
            {['Home', 'Explore', 'Private drops', 'Bookmarks', 'Settings'].map((item, index) => (
          ...
          <div className="mt-auto rounded-[24px] bg-[var(--lilac)] p-4 text-center"><Sparkles className="mx-auto text-primary" size={18} /><p className="mt-2 text-xs font-bold">Your ideas belong here</p></div>
        </aside>

        <section className="min-w-0 px-0 md:px-5" aria-label="PixAnony feed preview">
          <div className="flex items-center justify-between pb-4"><h2 className="text-xl font-bold">Your feed</h2><div className="flex gap-4 text-xs font-semibold"><span className="text-primary">Friends</span><span className="text-text-muted">Recent</span></div></div>
        ...
        <aside className="hidden border-l border-border/60 pl-4 md:block">
          ...
          {['Nora Wilde', 'Yusuf K.', 'Mai Chen'].map((name) => <div key={name} className="mb-3 flex items-center gap-2"><PixelAvatar username={name} size="xs" /><span className="min-w-0 flex-1 truncate text-xs font-semibold">{name}</span><span className="rounded-full bg-primary px-2 py-1 text-[10px] font-semibold text-primary-contrast">Follow</span></div>)}
          <div className="mt-8 rounded-[24px] bg-[var(--blush)] p-4"><Shield size={18} className="text-primary" /><p className="mt-3 text-sm font-bold">Anonymous when you want it</p><p className="mt-2 text-xs leading-5 text-text-muted">Send privately without revealing who you are.</p></div>
        </aside>
      </div>
    </div>
  );
  ...
  return (
    <section ref={heroRef} className="relative overflow-hidden pb-24 pt-36 sm:pt-44">
      ...
      <div className="site-container relative">
        ...
        >
          <p className="mx-auto mb-5 inline-flex items-center gap-2 rounded-full bg-card px-4 py-2 text-sm font-semibold shadow-card"><Sparkles size={15} className="text-primary" />A softer place to make and share</p>
          <h1 className="text-5xl font-bold leading-none tracking-[-0.05em] text-text sm:text-7xl">Make something small.<br /><span className="text-primary">Share something real.</span></h1>
...
function ScrollWord({ word, index, total, progress }: { word: string; index: number; total: number; progress: MotionValue<number> }) {
  ...
  return <motion.span style={reducedMotion ? undefined : { opacity, y }} className="mr-[.24em] inline-block">{word}</motion.span>;
}
...
function TaglineReveal() {
  ...
  return (
    <section ref={sectionRef} className="site-container py-24 sm:py-32">
      <p className="mb-5 text-sm font-bold text-primary">Expression on your terms</p>
    ...
    <motion.section id="benefits" className="site-container py-20" variants={revealGroup} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.18 }}>
      <div className="grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
        <motion.div variants={revealItem} className="rounded-[32px] bg-card p-8 shadow-card sm:p-12">
          <p className="text-sm font-bold text-primary">Why PixAnony</p>
          <h2 className="mt-4 max-w-xl text-4xl font-bold sm:text-5xl">Creativity feels better without performance pressure.</h2>
          <p className="mt-5 max-w-xl text-base leading-7 text-text-muted">Make work for the joy of making it. Share openly, send quietly, or keep exploring until inspiration arrives.</p>
        </motion.div>
  ...
  return (
    <section id="workflow" className="site-container py-20">
      <motion.div className="relative overflow-hidden rounded-[32px] bg-brand p-8 text-white sm:p-12" variants={revealGroup} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.25 }}>
        ...
        <motion.p variants={revealItem} className="relative text-sm font-bold text-white/70">How it works</motion.p>
        <motion.h2 variants={revealItem} className="relative mt-4 max-w-[680px] text-4xl font-bold sm:text-5xl">From blank canvas to meaningful connection.</motion.h2>
    ...
    <section id="founder" className="site-container py-20">
      <motion.div className="grid overflow-hidden rounded-[32px] bg-card shadow-card lg:grid-cols-[1.15fr_.85fr]" variants={revealGroup} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.25 }}>
        <motion.div variants={revealItem} className="p-8 sm:p-12">
          <p className="text-sm font-bold text-primary">Behind PixAnony</p>
          <h2 className="mt-4 max-w-2xl text-4xl font-bold sm:text-5xl">One focused creator, shaping one clear vision.</h2>
          <p className="mt-5 max-w-2xl text-base leading-7 text-text-muted">PixAnony is built independently with a simple belief: creativity should feel expressive, social, and safe enough to be anonymous when you choose.</p>
          <div className="mt-8 flex items-center gap-4">
            <PixelAvatar username="Nader Mohamed" size="xl" />
            <div>
              <h3 className="text-xl font-bold text-text">Nader Mohamed</h3>
              <p className="mt-1 text-sm font-semibold text-primary">Founder &amp; creator</p>
            </div>
          </div>
        </motion.div>
        <motion.div variants={revealItem} className="relative min-h-72 overflow-hidden bg-[var(--powder)] p-8 sm:p-12">
          ...
          <div className="relative flex h-full flex-col justify-between">
            <Logo size="md" />
            <p className="ml-auto max-w-xs text-right text-2xl font-bold leading-tight text-text">Designed, built, and cared for with a founder’s attention to every detail.</p>
          </div>
        </motion.div>
      </motion.div>
    </section>
    ...
    <motion.section id="questions" className="site-container py-20" variants={revealGroup} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }}>
      <div className="grid gap-10 lg:grid-cols-[.65fr_1.35fr]">
        <motion.div variants={revealItem}>
          ...
          <h2 className="mt-4 text-4xl font-bold sm:text-5xl">Good things to know.</h2>
          <p className="mt-5 text-base leading-7 text-text-muted">The short version: you stay in control of every piece and every interaction.</p>
        </motion.div>
    ...
    <footer className="site-container pb-8 pt-20">
      <div className="relative overflow-hidden rounded-[32px] bg-card p-8 shadow-card sm:p-12">
        ...
        <div className="relative flex flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
          <div><Logo /><h2 className="mt-8 max-w-xl text-3xl font-bold sm:text-5xl">Draw it your way.<br />Share it on your terms.</h2></div>
          <Link href="/login?mode=signup" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-primary px-6 text-base font-semibold text-primary-contrast">Start creating <ArrowRight size={17} /></Link>
        </div>
        <div className="relative mt-12 flex flex-wrap items-center gap-5 border-t border-border pt-6 text-sm font-semibold text-text-muted"><Link href="/explore">Explore</Link><Link href="/#founder">Founder</Link><Link href="/login">Sign in</Link><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><span className="ml-auto">© {new Date().getFullYear()} PixAnony</span></div>
      </div>
    </footer>
  );
}
