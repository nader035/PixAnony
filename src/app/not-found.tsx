  ...
  return (
    <main id="main-content" className="grid min-h-screen place-items-center bg-bg px-4 py-10 text-text">
      <div className="relative w-full max-w-xl overflow-hidden rounded-[32px] bg-card p-8 text-center shadow-float sm:p-12">
        ...
        <div className="relative"><Logo className="justify-center" /><span className="mx-auto mt-12 grid h-16 w-16 place-items-center rounded-full bg-[var(--powder)]"><Compass size={28} /></span><p className="mt-8 text-sm font-bold text-pink">404</p><h1 className="mt-3 text-4xl font-bold">{t('notFound.title')}</h1><p className="mx-auto mt-4 max-w-md text-base leading-7 text-text-muted">{t('notFound.description')}</p><Link href="/" className="mt-8 inline-flex min-h-12 items-center gap-2 rounded-full bg-primary px-6 text-base font-semibold text-bg"><ArrowLeft className="rtl-flip" size={16} />{t('notFound.backHome')}</Link></div>
      </div>
    </main>
  );
}
